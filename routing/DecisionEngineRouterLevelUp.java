 ...
 * @modified by vander, Sanata Dharma University
 */
/*
 ...
 */
public class DecisionEngineRouterLevelUp extends ActiveRouter
{
        public static final String PUBSUB_NS = "DecisionEngineRouterLevelUp";
	...
	protected boolean tombstoning;
      	protected RoutingDecisionEngineLevelUp levelUpDecider;
	...
	public DecisionEngineRouterLevelUp(Settings s)
	{
		super(s);
		...
		levelUpDecider = (RoutingDecisionEngineLevelUp)routeSettings.createIntializedObject(
	...
	public DecisionEngineRouterLevelUp(DecisionEngineRouterLevelUp r)
	{
		super(r);
		outgoingMessages = new LinkedList<Tuple<Message, Connection>>();
		levelUpDecider = (RoutingDecisionEngineLevelUp) r.levelUpDecider.replicate();
	...
	{
		return new DecisionEngineRouterLevelUp(this);
	}

	@Override
	public boolean createNewMessage(Message m)
	{
		if(levelUpDecider.newMessage(m))
		{
...
//				System.out.println("Host: " + getHost() + " Creating M7");
	...
	public void changedConnection(Connection con)
	{
		...
		DecisionEngineRouterLevelUp otherRouter = (DecisionEngineRouterLevelUp)otherNode.getRouter();
		if(con.isUp())
		{
			levelUpDecider.connectionUp(myHost, otherNode);
			...
			for(Message m : msgs)
			{
				if(levelUpDecider.shouldSendMessageToHost(m, otherNode))
					outgoingMessages.add(new Tuple<Message,Connection>(m, con));
			}
		}
		else
		{
			levelUpDecider.connectionDown(myHost, otherNode);
	...
	{
		conStates.put(con, 1);
		levelUpDecider.doExchangeForNewConnection(con, otherHost);
	}
	...
	protected int startTransfer(Message m, Connection con)
	{
		...
		else if(tombstoning && retVal == DENIED_DELIVERED)
		{
			...
			tombstones.add(m.getId());
				...
				levelUpDecider.shouldDeleteOldMessage(m, con.getOtherNode(getHost()))) {
...
//				System.out.println("Host: " + getHost() + " told to delete M7");
	...
	public Message messageTransferred(String id, DTNHost from)
	{
		...
		boolean isFinalRecipient = levelUpDecider.isFinalDest(aMessage, getHost());
		boolean isFirstDelivery =  isFinalRecipient && 
			!isDeliveredMessage(aMessage);
		
		if (outgoing!=null && levelUpDecider.shouldSaveReceivedMessage(aMessage, getHost())) 
		{
	...
	protected void transferDone(Connection con)
	{
		...
		if(levelUpDecider.shouldDeleteSentMessage(transferred, con.getOtherNode(getHost())))
		{
//			if(transferred.getId().equals("M7"))
	...
	public RoutingDecisionEngineLevelUp getDecisionEngine()
	{
		return this.levelUpDecider;
	}
...
//		for(Connection c : getHost()) 
		for(Connection c : getConnections())
		{
			...
			if(other != from && levelUpDecider.shouldSendMessageToHost(m, other))
			{
//				if(m.getId().equals("M7"))
